...
// Last register + 1
#define TMP_REGISTER	(SLJIT_STACK_PTR_REG + 1)
...
#define BINARY_IMM(_op_imm_, _op_mr_, immw, arg, argw) \
	if (IS_HALFWORD(immw) || compiler->mode32) { \
		code = emit_x86_bin_instruction(compiler, 1, SLJIT_IMM, immw, arg, argw); \
		TEST_MEM_ERROR(code); \
		*(code + 1) |= (_op_imm_); \
	} \
	else { \
		if (emit_load_imm64(compiler, TMP_REG2, immw)) \
			return compiler->error; \
		code = emit_x86_bin_instruction(compiler, 1, TMP_REG2, 0, arg, argw); \
		TEST_MEM_ERROR(code); \
		*code = (_op_mr_); \
	}
	...
	int src2, sljit_w src2w)
{
	...
	if (dst == src1 && dstw == src1w) {
		...
		else if (src2 >= SLJIT_TEMPORARY_REG1 && src2 <= SLJIT_STACK_PTR_REG) {
	...
	if (dst == src2 && dstw == src2w) {
		...
		else if (src1 >= SLJIT_TEMPORARY_REG1 && src1 <= SLJIT_STACK_PTR_REG) {
	...
	int src2, sljit_w src2w)
{
...
#else
		EMIT_MOV(compiler, SLJIT_MEM1(SLJIT_STACK_PTR_REG), -(int)sizeof(sljit_w), SLJIT_PREF_MUL_DST, 0);
#endif
	}
	...
	if (src1 != SLJIT_PREF_MUL_DST && depends_on(src2, SLJIT_PREF_MUL_DST)) {
		...
		EMIT_MOV(compiler, SLJIT_MEM1(SLJIT_STACK_PTR_REG), -(int)(sizeof(sljit_w) * 2), SLJIT_TEMPORARY_REG2, 0);
#endif
...
#ifdef SLJIT_CONFIG_X86_32
		EMIT_MOV(compiler, SLJIT_TEMPORARY_REG2, 0, SLJIT_MEM1(SLJIT_STACK_PTR_REG), -(int)(sizeof(sljit_w) * 2));
#endif
	}
	else {
#ifdef SLJIT_CONFIG_X86_32
...
#else
			EMIT_MOV(compiler, SLJIT_PREF_MUL_DST, 0, SLJIT_MEM1(SLJIT_STACK_PTR_REG), -(int)sizeof(sljit_w));
#endif
			EMIT_MOV(compiler, dst, dstw, TMP_REGISTER, 0);
		}
		else {
	...
	int src2, sljit_w src2w)
{
	...
	if (src1 >= SLJIT_TEMPORARY_REG1 && src1 <= SLJIT_STACK_PTR_REG) {
		if (src2 & SLJIT_IMM) {
	...
	if (src2 >= SLJIT_TEMPORARY_REG1 && src2 <= SLJIT_STACK_PTR_REG && !(src1 & SLJIT_IMM)) {
	...
	int src2, sljit_w src2w)
{
		...
		if (src2 & SLJIT_IMM) {
			...
			if (IS_HALFWORD(src2w) || compiler->mode32) {
				...
				*code = 0xf7;
				...
				if (emit_load_imm64(compiler, TMP_REG2, src2w))
					return compiler->error;
				code = emit_x86_bin_instruction(compiler, 1, TMP_REG2, 0, src1, 0);
				TEST_MEM_ERROR(code);
				*code = 0x85;
			}
#else
	...
	if (src2 >= SLJIT_TEMPORARY_REG1 && src2 <= SLJIT_STACK_PTR_REG) {
		if (src1 & SLJIT_IMM) {
#ifdef SLJIT_CONFIG_X86_64
			if (IS_HALFWORD(src1w) || compiler->mode32) {
				...
				*code = 0xf7;
				...
				if (emit_load_imm64(compiler, TMP_REG2, src1w))
					return compiler->error;
				code = emit_x86_bin_instruction(compiler, 1, TMP_REG2, 0, src2, 0);
				TEST_MEM_ERROR(code);
				*code = 0x85;
			}
#else
	...
	if (src2 & SLJIT_IMM) {
		...
		if (IS_HALFWORD(src2w) || compiler->mode32) {
			...
			*code = 0xf7;
			...
			if (emit_load_imm64(compiler, TMP_REG2, src2w))
				return compiler->error;
			code = emit_x86_bin_instruction(compiler, 1, TMP_REG2, 0, TMP_REGISTER, 0);
			TEST_MEM_ERROR(code);
			*code = 0x85;
		}
#else
	...
	int src2, sljit_w src2w)
{
	...
	if (dst == SLJIT_PREF_SHIFT_REG) {
		...
		*code |= mode;
		...
		EMIT_MOV(compiler, SLJIT_PREF_SHIFT_REG, 0, src2, src2w);
		code = emit_x86_shift_instruction(compiler, SLJIT_PREF_SHIFT_REG, 0, dst, 0);
		TEST_MEM_ERROR(code);
		*code |= mode;
		EMIT_MOV(compiler, SLJIT_PREF_SHIFT_REG, 0, TMP_REGISTER, 0);
	}
...
#else
		EMIT_MOV(compiler, SLJIT_MEM1(SLJIT_STACK_PTR_REG), -(int)sizeof(sljit_w), SLJIT_PREF_SHIFT_REG, 0);
#endif
...
#else
		EMIT_MOV(compiler, SLJIT_PREF_SHIFT_REG, 0, SLJIT_MEM1(SLJIT_STACK_PTR_REG), -(int)sizeof(sljit_w));
#endif
		EMIT_MOV(compiler, dst, dstw, TMP_REGISTER, 0);
	}
